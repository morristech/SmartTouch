...
public class SideBar extends View
{
	...
	{
		this.paint.setTextSize(TextSize*getContext().getResources().getDisplayMetrics().density);
	...
	private float getLetterHeight(float singleHeight)
	{
		...
		for(int i=16;i>=0;i--)
		{
			iHeight = MeasureStringHeight("A",i);
			if(iHeight<singleHeight-10)
			{
				return i*getContext().getResources().getDisplayMetrics().density;
			}
		}
		return 10*getContext().getResources().getDisplayMetrics().density;
	}
	...
	public boolean dispatchTouchEvent(MotionEvent event)
	{
		...
		default:
			setBackgroundResource(R.drawable.smartkey_sidebar_background);
			if (oldChoose != c)
			{
				if (c >= 0 && c < b.length)
				{
