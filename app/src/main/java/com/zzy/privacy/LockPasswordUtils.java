...
public class LockPasswordUtils
{
	...
	public void saveLockPassword(String password, int quality, boolean isFallback)
	{
		...
		catch (FileNotFoundException fnfe)
		{
			...
			// provider
			Common.LogEx("Unable to save lock pattern to " + sLockPasswordFilename);
		}
	...
	public byte[] passwordToHash(String password)
	{
		...
		{
			Common.LogEx("Failed to encode string because of missing algorithm: " + algo);
		}
		return hashed;
	}
	...
	private String getSalt()
	{
		...
		if (salt == 0)
		{
			try
			{
				...
				Common.LogEx("Initialized lock password salt");
			}
			catch (NoSuchAlgorithmException e)
			{
