...
public class LockPatternUtils {
	...
	private static class LockPatternFileObserver extends FileObserver {
		...
		public void onEvent(int event, String path) {
			Common.LogEx("file path" + path);
			if (LOCK_PATTERN_FILE.equals(path)) {
				Common.LogEx("lock pattern file changed");
	 ...
	 */
	public void saveLockPattern(List<LockPatternView.Cell> pattern,File file) {
		// Compute the hash
		...
		catch (FileNotFoundException fnfe) 
		{
			...
			// provider
			Common.LogEx("Unable to save lock pattern to " + sLockPatternFilename);
	...
	public void saveLockPatternDefault(List<LockPatternView.Cell> pattern)
	{
		saveLockPattern(pattern,sLockPatternFilename);
	}
	
	/*
	 * Generate an MD5 hash for the pattern. Not the most secure, but it is at
	...
	{
		return GetPattern(sLockPatternFilename);
	}
	...
	public String GetPattern(File file)
	{
		try
		{
			...
			String sByte = Base64.encodeToString(stored,Base64.DEFAULT);
			return sByte;
		} 
		catch (FileNotFoundException fnfe)
		{
	...
	public void saveLockPatternByHash(byte[] hash)
	{
		try
		{
		...
		{
			Common.LogEx("Unable to save lock hash to "+ sLockPatternFilename);
		} 
		catch (IOException ioe)
		{
			Common.LogEx("Unable to save lock hash to "+ sLockPatternFilename);
